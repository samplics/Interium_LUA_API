...
local function RenderSpeed()
	...
	if (GetBool(Vars.misc_jumpbug) and (Globals.IsJumpBug())) then
        Render.Text_TahomaBd("JB", (Globals.ScreenWeight()) / 2, (Globals.ScreenHeight()) / 2 + (25 + Pos[0]), 24, Color.new(255, 50, 50, 255), true, true)
    end

	if (GetBool(Vars.misc_autostrafe) and GetInt(Vars.misc_autostrafe_enabletype) and (Globals.IsAutoStrafe())) then
        Render.Text_TahomaBd("AS", (Globals.ScreenWeight()) / 2, (Globals.ScreenHeight()) / 2 + (25 + Pos[1]), 24, Color.new(50, 255, 50, 255), true, true)
    end

	if (GetBool(Vars.misc_autoduck) and GetInt(Vars.misc_autoduck_enabletype) and (Globals.IsAutoDuck())) then
        Render.Text_TahomaBd("AD", (Globals.ScreenWeight()) / 2, (Globals.ScreenHeight()) / 2 + (25 + Pos[2]), 24, Color.new(50, 50, 255, 255), true, true)
    end
end
