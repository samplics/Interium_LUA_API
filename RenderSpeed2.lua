...
-- Customize Pos
local SpeedPos = Vector2D.new(Screen.x / 2, Screen.y - Screen.y / 4 + 125)
local GraphPos = Vector2D.new(Screen.x / 2 - SpeedArraySize / 2, Screen.y - Screen.y / 4 + 50)
...
local function RenderSpeed()
    ...
    Render.Text_TahomaBd(tostring(iSpeed), SpeedPos.x, SpeedPos.y, 24, col, true, true)
   ...
   -- For Only Air
    ...
    for i = 2, SpeedArraySize do
        if (SpeedArray[i]) then
            ...
            Render.Line(GraphPos.x + i - 1, GraphPos.y + SpeedArray[i - 1], GraphPos.x + i, GraphPos.y + SpeedArray[i], col2, 1)
        else
            break
        end
    end
